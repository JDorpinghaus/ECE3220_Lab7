...
int main()
{
	...
	obj1 + obj2;	// normal expression. Which operator is being called? Lab7::operator+
	// obj1.operator+(obj2);	// equivalent, try it out
	obj1.Printvals();
	obj2.Printvals();

	obj2 + obj1;	// normal expression. Which operator is being called? Lab7::operator+
	...
	cout << "------------------------------------------------------------------" << endl;
	// Which operator is being called next? The non-member operator
	obj3 = operator+(obj1, obj2);	// Would  obj3 = obj2 + obj1; work?
	//obj3 = obj2 + obj1;
	obj1.Printvals();				// If not, how could you make it work? Define a operator= function
	obj2.Printvals();
	obj3.Printvals();
